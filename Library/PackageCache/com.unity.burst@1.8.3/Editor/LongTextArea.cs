...
namespace Unity.Burst.Editor
{
    internal class LongTextArea
    {
        ...
        private readonly Color _lineHighlightColor = new Color(.56f, .56f, .56f, 1f);
        ...
        public bool Render(GUIStyle style, Rect workingArea, bool showBranchMarkers, bool doSearch, SearchCriteria searchCriteria, Regex regx)
        {
            ...
            else
            {
                ...
                RenderLineHighlight(workingArea, _actualHorizontalPad);
            }
        ...
        private int GetLinesBlockIdx(int pressedLine)
        {
        ...
        internal Rect GetLineHighlight(ref LineRegRectsCache cache, float hPad, int pressedLine)
        {
            ...
            var xStart = hPad;
            var rect = new Rect(xStart, positionY + fontHeight - 2f, xEnd, lineHeight);
            cache.UpdateLineHighlight(pressedLine, rect, _folded[b]);
            return rect;
        }

        private void RenderLineHighlight(Rect workingArea, float hPad)
        {
            if (_pressedLine == -1)
            {
                return;
        ...
        private LineRegRectsCache _lineRegCache;
