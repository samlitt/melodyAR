...
public class LongTextAreaTests
{
    ...
    public void KeepingSelectionWhenMovingTest(bool useDisassembler)
    {
        ...
        Assert.That(start + new Vector2(_textArea.fontWidth, 0), Is.EqualTo(_textArea.selectDragPos).Within(precision),
            "Text selection did not match after right movement.");

        _textArea.MoveSelectionLeft(workingArea, true);
        Assert.IsTrue(_textArea.HasSelection);
        Assert.That(start, Is.EqualTo(_textArea.selectDragPos).Within(precision),
            "Text selection did not match after left movement.");

        // Vertical movement:
        _textArea.MoveSelectionDown(workingArea, true);
        Assert.IsTrue(_textArea.HasSelection);
        Assert.That(start + new Vector2(0, _textArea.fontHeight), Is.EqualTo(_textArea.selectDragPos).Within(precision),
    ...
    */
    ...
    public void GetLineHighlightTest()
    {
        ...
        const string compileTargetName = "BurstInspectorGUITests.MyJob - (IJob)";
        // Get target job assembly:
         var disassembler = GetDisassemblerandText(compileTargetName, 2, out var textToRender);
         ...
         var expectedY = (_textArea.blockLine[b] + (linePressed - disassembler.Blocks[b].LineIndex)) * _textArea.fontHeight + _textArea.fontHeight - 2f;
    ...
    public void GetRegRectsTest()
    {
        ...
        Assert.AreEqual(expectedX,  rects[0][rects[0].Count - 1].x, "Wrong x position for last found \"rbp\" rect.");
